...
function* handleUpdateSection(action) { try { yield new Promise(r => setTimeout(r, API_DELAY)); yield put(updateSectionSuccess(action.payload)); } catch (err) { yield put(saveError(err.message)); } }
function* handleDeleteSection(action) { try { yield new Promise(r => setTimeout(r, API_DELAY)); yield put(deleteSectionSuccess(action.payload)); } catch (err) { yield put(saveError(err.message)); } }
function* handleReorderSections(action) { try { yield new Promise(r => setTimeout(r, 300)); yield put(reorderSectionsSuccess(action.payload)); } catch (err) { yield put(saveError(err.message)); } }

function* handleAddTask(action) { try { yield new Promise(r => setTimeout(r, API_DELAY)); const { sectionId, title, description } = action.payload; const newTask = { id: 'task-' + generateId(), title, description: description || '', status: sectionId, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; yield put(addTaskSuccess({ sectionId, task: newTask })); } catch (err) { yield put(saveError(err.message)); } }

function* handleUpdateTask(action) { try { yield new Promise(r => setTimeout(r, API_DELAY)); const updates = { ...action.payload.updates, updatedAt: new Date().toISOString() }; yield put(updateTaskSuccess({ sectionId: action.payload.sectionId, taskId: action.payload.taskId, updates })); } catch (err) { yield put(saveError(err.message)); } }
function* handleDeleteTask(action) { try { yield new Promise(r => setTimeout(r, API_DELAY)); yield put(deleteTaskSuccess(action.payload)); } catch (err) { yield put(saveError(err.message)); } }
function* handleMoveTask(action) { try { yield new Promise(r => setTimeout(r, 300)); yield put(moveTaskSuccess(action.payload)); } catch (err) { yield put(saveError(err.message)); } }
function* handleReorderTasks(action) { try { yield new Promise(r => setTimeout(r, 300)); yield put(reorderTasksSuccess(action.payload)); } catch (err) { yield put(saveError(err.message)); } }
